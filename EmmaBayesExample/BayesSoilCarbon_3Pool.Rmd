---
title: "Bayesian Soil Carbon Modeling: 3-Pool Tutorial"
...
# Analytical rectangles for uniform prior
prior_rect_compare <- data.frame(
  parameter = factor(par_names, levels = par_names),
  xmin = prior_lower,
  xmax = prior_upper,
  ymax = 1 / (prior_upper - prior_lower)
)

ggplot() +
  geom_rect(data = prior_rect_compare,
            aes(xmin = xmin, xmax = xmax, ymin = 0, ymax = ymax),
            fill = "skyblue", alpha = 0.4) +
  geom_density(data = prior_long_gauss, aes(x = value),
               fill = "coral", alpha = 0.4, linewidth = 0.6) +
  facet_wrap(~ parameter, scales = "free", ncol = 3) +
  labs(title = "Uniform (blue) vs. Gaussian (coral) Priors",
...
# Analytical uniform prior density as rectangles
prior_rect_df <- data.frame(
  parameter = factor(par_names, levels = par_names),
  xmin = prior_lower,
  xmax = prior_upper,
  ymax = 1 / (prior_upper - prior_lower)
