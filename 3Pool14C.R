# =============================================================================
# Three-Pool Soil Carbon Model with Radiocarbon (14C) Dynamics
# =============================================================================
#
# This script demonstrates three multi-pool soil carbon models with radiocarbon
...
# Decomposition rate constants (yr^-1) for three pools
...
# =============================================================================
# PART 4: Three-Pool Parallel Model
# =============================================================================
...
#                /  |  \
#               v   v   v
#            [P1] [P2] [P3]  (independent pools)
#               \   |   /
#                v  v  v
#              Respiration
#
# gam1, gam2: Fraction of litter input allocated to pools 1 and 2
# (Pool 3 receives 1 - gam1 - gam2 = 0.2)

Parallel <- ThreepParallelModel14(
  t = years,
  ks = k,
  C0 = C0,
  F0_Delta14C = c(0, 0, 0),  # Initial 14C signature of each pool (pre-bomb = ~0‰)
  In = LitterInput,
  gam1 = 0.6,                # 60% of input goes to fast pool
  gam2 = 0.2,                # 20% of input goes to intermediate pool
  inputFc = Fatm             # Atmospheric 14C forcing
)

# =============================================================================
# PART 5: Three-Pool Series Model
# =============================================================================
# In a series model, carbon flows sequentially through the pools:
# fast → intermediate → slow. This represents progressive stabilization
# of organic matter as it ages.
#
# Structure:  Atmosphere
#                 |
#                 v
...
# a32: Transfer coefficient from pool 2 to pool 3 (fraction of k2 * C2)

a21 <- 0.9 * k[1]  # 90% of decomposed fast pool C transfers to intermediate pool
a32 <- 0.4 * k[2]  # 40% of decomposed intermediate pool C transfers to slow pool
...
# =============================================================================
# PART 6: Three-Pool Feedback Model
# =============================================================================
# The feedback model extends the series model by allowing carbon to cycle
# back from slower to faster pools. This represents processes like microbial
...
#               Respiration (from each pool)
#
# a12, a23: Backward transfer coefficients (feedback from slower to faster)

a12 <- 0.4 * k[2]  # 40% of decomposed intermediate pool C returns to fast pool
a23 <- 0.7 * k[3]  # 70% of decomposed slow pool C returns to intermediate pool
...
create_pool_df <- function(years, pool_data, model_name) {
  data.frame(
    Year = rep(years, 3),
    Delta14C = c(pool_data[, 1], pool_data[, 2], pool_data[, 3]),
    Pool = rep(c("Pool 1 (fast)", "Pool 2 (intermediate)", "Pool 3 (slow)"), each = length(years)),
    Model = model_name
  )
}
...
p_pools + p_bulk +
  plot_annotation(
    title = "Three-Pool Radiocarbon Model Comparison",
